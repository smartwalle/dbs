...
func TestUnionBuilder(t *testing.T) {
	...
	check(
		t,
		ub,
		"(SELECT p1.id, p1.name, p1.price FROM products AS p1) UNION ALL (SELECT p2.id, p2.name, p2.price FROM products AS p2) ORDER BY price",
		[]interface{}{},
	)
}
